...
public class orderrepository {
	...
	public void saveOrderDetail(orderdetail def,CustomerEntity cs) {
		...
		String sql1="INSERT INTO TBL_REVENUE(MENU_NAME,AMOUNT,PRICE,ALLPRICE,TAX,REVENUE,WDATE,CUSTOMER_NAME,ADDRESS) VALUES(?,?,?,?,?,?,?,?,?)";
		Date date = new Date();
		int year = date.getYear()+1900;
		int month = date.getMonth()+1;
		int day = date.getDate();
		jdbc.update(sql1,def.getMenu(),def.getAmount(),def.getPrice(),def.getAllprice(),def.getTax(),def.getRevenue(), year+"-"+month+"-"+day,cs.getName(),cs.getAddress());
